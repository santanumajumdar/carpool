...
class Pages extends Admin_Controller
{
	...
	function index()
	{
		...
		$config['paging_function'] 	= 'page_ajax';
		$config['base_url'] 		=  site_url('admin/pages');
		$data['count_result'] 		= $this->Page_model->count_pages(); 
		...
		$data['pages']	= $this->Page_model->get_allpages($this->pagination_admin->per_page,$this->uri->segment(4));
	...
	function page_ajax()
	{
		...
		$this->load->view($this->config->item('admin_folder').'/page-list', $data);
	}
	...
	********************************************************************/
	function form($id = false)
	{
		...
		else
		{
			...
			$route['id']	= $route_id;
			$route['slug']	= $slug;
			$route['route']	= 'pages/page/'.$page_id;
