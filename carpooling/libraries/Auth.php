...
class Auth
{
	...
	*/
	function login_admin($email, $password, $remember=false)
	{
		...
		if (sizeof($result) > 0)
		{
			...
			$admin['admin']['name']	= $result['admin_name'];
			$admin['admin']['email']	= $result['admin_email'];
	...
	*/
	function reset_password($email)
	{
		...
		if ($admin)
		{
			...
			$admin['admin_password']	= sha1($new_password); 
			$this->save($admin);
			
			$res = $this->CI->db->where('tplid', '26')->get('tbl_email_template');
			$row = $res->row_array();
			 
			
	    
			$row['tplmessage'] = str_replace('{EMAIL}', $admin['admin_email'], $row['tplmessage']);
			 
			$row['tplmessage'] = str_replace('{PASSWORD}', $new_password, $row['tplmessage']);

			$param['message'] = $row['tplmessage'];

			$email_subject = $this->CI->load->view('template', $param, TRUE);
			$config['mailtype'] = 'html';

			$this->CI->email->initialize($config);

			$this->CI->email->from($this->CI->config->item('email'), $this->CI->config->item('site_name'));
			$this->CI->email->to($admin['admin_email']);
			$this->CI->email->subject($this->CI->config->item('site_name').': Admin Password Reset');
			$this->CI->email->message(html_entity_decode($email_subject));
			$this->CI->email->send();
	...
	*/
	function get_admin_list()
	{
		...
		$this->CI->db->order_by('admin_name', 'ASC');
		$this->CI->db->order_by('admin_email', 'ASC');
		$result = $this->CI->db->get('tbl_admin');
	...
	*/
	...
	function delete($id)
	{
		if ($this->check_id($id))
		{
			...
			$this->CI->db->where('id', $id);
			$this->CI->db->limit(1);
			$this->CI->db->delete('tbl_admin');

			return $admin->admin_name.' .has been removed.';
		}
		else
		{
