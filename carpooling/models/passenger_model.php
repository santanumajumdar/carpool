...
Class Passenger_model extends CI_Model
{
	...
	********************************************************************/
	...
	function get_passengers($limit=0, $offset=0, $order_by='passenger_id', $direction='DESC')
	{
		...
		$result	= $this->db->get('tbl_passengers');
		
		return $result->result();
	}
	
	function get_passenger($userid)
	{
		
		$result	= $this->db->get_where('tbl_passengers', array('passenger_id'=>$userid));
		return $result->row();
	}
	...
	{
		if ($provider['passenger_id'])
		{
			$this->db->where('passenger_id', $provider['passenger_id']);
			$this->db->update('tbl_passengers', $provider);
			return $provider['passenger_id'];
		}
		else
		{
			$this->db->insert('tbl_passengers', $provider);
	...
	function delete($prodid)
	{
		...
		*/
		...
		$this->db->where('passenger_id', $prodid);
		$this->db->delete('tbl_passengers');
...
//		$this->db->delete('orders');
	...
	function check_email($str, $prodid=false)
	{
		
	
		$this->db->select('passenger_email');
		$this->db->from('tbl_passengers');
		$this->db->where('passenger_email', $str);
		if ($prodid)
		{
			$this->db->where('passenger_id !=', $prodid);
		}
